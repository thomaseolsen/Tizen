...
void connection_view3_next3_onclicked(uib_view3_view_context*, Evas_Object*, void*);
...
uib_view_context* uib_view_view3_create(Evas_Object* parent, void* create_callback_param) {
	...
	//bind event handler
	evas_object_smart_callback_add(vc->prev3, "clicked", (Evas_Smart_Cb)connection_view3_button1_onclicked, vc);

	evas_object_smart_callback_add(vc->next3, "clicked", (Evas_Smart_Cb)connection_view3_next3_onclicked, vc);
...
void uib_view3_config_CIRCLE_360x360_portrait() {
	...
	if(vc->grid1) {
		...
		if (!vc->prev3) {
			vc->prev3 = elm_button_add(vc->grid1);
		}
		if (vc->prev3) {
			evas_object_size_hint_align_set(vc->prev3, -1.0, -1.0);			evas_object_size_hint_weight_set(vc->prev3, 1.0, 1.0);			elm_object_text_set(vc->prev3,_UIB_LOCALE("Previous"));
			elm_object_style_set(vc->prev3,"bottom");
			evas_object_show(vc->prev3);
		}
		if (!vc->box1) {
		...
		if (!vc->next3) {
			vc->next3 = elm_button_add(vc->grid1);
		}
		if (vc->next3) {
			evas_object_size_hint_align_set(vc->next3, -1.0, -1.0);			evas_object_size_hint_weight_set(vc->next3, 1.0, 1.0);			elm_object_text_set(vc->next3,_UIB_LOCALE("Next"));
			elm_object_style_set(vc->next3,"bottom");
			evas_object_show(vc->next3);
		}
		elm_grid_pack(vc->grid1, vc->prev3, 100, 100, 800, 80);		elm_grid_pack(vc->grid1, vc->box1, 0, 265, 1000, 435);		elm_grid_pack(vc->grid1, vc->next3, 100, 850, 800, 80);		evas_object_show(vc->grid1);
	}
}
