...
static void _time_elapse_tick_cb(int start_time, int current_time);
static void _time_elapse_end_cb(void);
...
{
	view_create();
	data_initialize(_time_elapse_tick_cb, _time_elapse_end_cb);
...
/**
 * @brief Internal callback function invoked for each timer's time tick.
 * This function is responsible for displayed time update.
 * @param[in] time The remaining time in seconds.
 */
static void _time_elapse_tick_cb(int start_time, int current_time)
{
	view_update_time_display(start_time, current_time);
}

/**
 * @brief Internal callback function invoked when the timer's time is elapsed.
 * This function is responsible for displayed time update.
 */
static void _time_elapse_end_cb(void)
{
